...
module.exports = function(conn) {
    ...
    api.buildings = function(req, res) {
        conn.query('SELECT room.id as room_id, room.building_id, building.name as building_name, room.name as room_name FROM room JOIN building on room.building_id=building.id JOIN campus ON building.campus_id=campus.id WHERE campus.school_id=?', [req.params.sid], function(err, results) {
            results = _.groupBy(results, 'building_id');
            res.json(results);
        });
    };

    api.colleges = function(req, res) {
        conn.query('SELECT * FROM college WHERE school_id=?', [req.params.sid], function(err, results) {
            res.json(results);
        });
    };

    api.departments = function(req, res) {
        conn.query('SELECT * FROM department JOIN college ON department.college_id=college.id WHERE college.school_id=?', [req.params.sid], function(err, results) {
            res.json(results);
        });
    };

    api.subjects = function(req, res) {
        conn.query('SELECT * FROM subject WHERE school_id=?', [req.params.sid], function(err, results) {
            res.json(results);
        });
    };
