...
function whereIn(iarr, key, carr) {
    // filter the inarray by whether or not that key in conditional array
...
function whereClassInCart(iarr, carr) {
    // filter the inarray by whether or not that key in conditional array
    return _.filter(iarr, function(t) {
        // return key in array
        return carr.indexOf(t.class.id) !== -1;
    });
}

function getAllCourses(conn, cb) {
    var r = cache.get('getAllCourses');
    if (r !== null) {
        cb(r);
    }
    conn.query('SELECT * FROM course', function(err, results) {
        ...
        cb(results);
        cache.put('getAllCourses', results);
    });
}
...
module.exports = function(conn) {
    ...
    api.search = function(req, res) {
        var sid = req.params.sid,
            ...
            shouldFilter = req.query.shouldFilter || false;
        ...
        if (q !== '') {
            ...
            var mq = q.replace(' ', '-');
            //  OR (course.desc LIKE (?))
            conn.query('SELECT * FROM course WHERE (name LIKE ?) OR (code LIKE ?) OR (code LIKE ?)', [wrap(q), wrap(q), wrap(mq)], function(err, results) {
                if (err) {
                    console.log(err);
                }
                results = results.map(function(c) {
            ...
            .then(function searchTeachers(courses) {
                ...
                conn.query('SELECT * FROM teacher WHERE teacher.name LIKE ?', [wrap(q)], function(err, teachers) {
                    if (teachers.length > 0) {
                        ...
                        conn.query('SELECT section, class.id as id, teacher.id as teacher_id, course.code as course_code, course.id as course_id FROM class JOIN teacher ON class.teacher_id=teacher.id JOIN course ON class.course_id=course.id WHERE class.teacher_id IN (?)', [tids], function(err, results) {
                            var classes_by_teacher_id = _.groupBy(results, 'teacher_id');
                            for (var i = 0; i < teachers.length; i++) {
                                // for each teacher, add their classes
                                teachers[i].classes = classes_by_teacher_id[teachers[i].id];
                            }
        ...
        deferred.promise.then(function getBuildings(courses) {
            var deferred = Q.defer();
            if (_.isEmpty(courses) || !shouldFilter) {
                deferred.resolve(courses);
                return deferred.promise;
            }
            ...
            if (qbuildings !== undefined) {
                ...
                if (qrooms !== undefined) {
                    ...
                    conn.query('SELECT course_id FROM class JOIN room ON class.room_id=room.id JOIN building ON room.building_id=building.id WHERE class.room_id IN (?) AND room.building_id IN (?) GROUP BY course_id', [qrooms, qbuildings], function(err, results) {
                        if (err) {
                            console.log(err);
                        }
                    ...
                    // only buildings
                    conn.query('SELECT course_id FROM class JOIN room ON class.room_id=room.id JOIN building ON room.building_id=building.id WHERE room.building_id IN (?) GROUP BY course_id', [qbuildings], function(err, results) {
                        if (err) {
                            console.log(err);
                        }
                        var cids = _.pluck(results, 'course_id');
        ...
        .then(function getCampus(courses) {
            ...
            if (qcampuses !== undefined) {
                conn.query('SELECT course_id FROM course JOIN class ON class.course_id=course.id JOIN room ON class.room_id=room.id JOIN building ON room.building_id=building.id WHERE campus_id IN (?) GROUP BY course_id', [qcampuses], function(err, results) {
                    if (err) {
                        console.log(err);
                    }
                    var cids = _.pluck(results, 'course_id');
        ...
        .then(function getCollege(courses) {
            ...
            if (qcolleges !== undefined) {
                conn.query('SELECT course.id as id FROM course JOIN subject ON course.subject_id=subject.id JOIN department ON subject.department_id=department.id WHERE college_id IN (?)', [qcolleges], function(err, results) {
                    if (err) {
                        console.log(err);
                    }
        ...
        //     }
        //     if (qdepts !== undefined) {
        //         conn.query('SELECT course.id as id FROM course JOIN subject ON course.subject_id=subject.id WHERE department_id IN (?)', [qdepts], function(err, results) {
        //             if (err) {
        //                 console.log(err);
        //             }
        //             var cids = _.pluck(results, 'id');
        ...
        .then(function getSubjects(courses) {
            ...
            if (qsubjects !== undefined) {
                conn.query('SELECT course.id as id FROM course WHERE subject_id IN (?)', [qsubjects], function(err, results) {
                    if (err) {
                        console.log(err);
                    }
                    var cids = _.pluck(results, 'id');
        ...
        .then(function getTeachers(courses) {
            ...
            if (qteachers !== undefined) {
                conn.query('SELECT course_id FROM class JOIN teacher ON class.teacher_id=teacher.id WHERE teacher.id IN (?) GROUP BY course_id', [qteachers], function(err, results) {
                    if (err) {
                        console.log(err);
                    }
                    var cids = _.pluck(results, 'course_id');
        ...
        .then(function(courses) {
            ...
            if (qterms !== undefined) {
                conn.query('SELECT course_id FROM class WHERE term_id IN (?) GROUP BY course_id', [qterms], function(err, results) {
                    if (err) {
                        console.log(err);
                    }
                    var cids = _.pluck(results, 'course_id');
    ...
    function friendTakesCourse(friend, course) {
        for (var i = 0; i < friend.mutual.length; i++) {
            if (friend.mutual[i].class.id === course.class.id) {
                return true;
            }
        }
    ...
    api.compare = function(req, res) {
        User.findOne({
            ...
            FB.api('/me/friends', {
                ...
                User.find({
                    'facebook.id': {$in: _.pluck(response.data, 'id')}
                }, function(err, friends) {
                    var myCids = _.pluck(_.pluck(user.state.cart, 'class'), 'id');
                    friends = friends.map(function(friend) {
                        var cids = _.pluck(_.pluck(friend.state.cart, 'class'), 'id');
                        return {
                            id: friend.facebook.id,
                            name: friend.facebook.name,
                            mutual: whereClassInCart(user.state.cart, _.intersection(myCids, cids)),
                            link: friend.facebook.link
                        };
                    });

                    var courses = [];
                    _.forEach(user.state.cart, function(course) {
                        // for each course
                        // determine if friend is in that course by comparing course ID and class id
                        var cs = [];
                        _.forEach(friends, function(friend) {
                            if (friendTakesCourse(friend, course)) {
                                cs.push(friend);
                            }
                        });
                        if (cs.length) {
                            courses.push(_.merge(course, {mutual: cs}));
                        }
                    });
