    ...
    api.search = function(req, res) {
        ...
        .then(function getCampus(courses) {
            ...
            if (qcampuses !== undefined) {
                conn.query('SELECT course_id FROM class JOIN room ON class.room_id=room.id JOIN building ON room.building_id=building.id JOIN campus ON building.campus_id=campus.id WHERE campus.id IN (?) GROUP BY course_id', [qcampuses], function(err, results) {
                    if (err) {
                        console.log(err);
                    }
                    var cids = _.pluck(results, 'course_id');
